...
Login
    ...
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Login
    [Tags]    LOCALIZA
    CT: Logar no "${LOCALIZA_TEST_ENVIROMENT}" com sucesso
    Sleep    1s
...
Administradores
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Administradores
    CT: Usuários/administradores
    Sleep    1s
    Stop Video Recording

Endereços bloqueados
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Endereços bloqueados
    CT: Usuários/Endereços bloqueados
    Sleep    1s
    Stop Video Recording

Grupo de permissões
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Grupo de permissões
    CT: Usuários / Grupo de permissões
    Sleep    1s
    Stop Video Recording

Logs dos usuários
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Logs dos usuários
    CT: Usuários/Logs de usuários
    Sleep    1s
    Stop Video Recording

Transações
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Transações
    CT: Usuários/Transações
    Sleep    1s
    Stop Video Recording

Push notifications
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Push Notifications
    CT: Marketing/Push Notifications
    Sleep    1s
    Stop Video Recording

Feriados
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Feriados
    CT: Configurações / Feriados
    Sleep    1s
    Stop Video Recording

Gestão de bancos
    Start Video Recording    name=./LOCALIZA/${TODAY}/BACKOFFICE/Gestão_bancos
